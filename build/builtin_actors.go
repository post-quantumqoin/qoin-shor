...
func init() {
	// fmt.Println("builtin_actors.go NetworkBundle:", NetworkBundle)
...
func loadManifests(netw string) error {
	...
	// fmt.Println("loadManifests Network:", netw)
...
func GetEmbeddedBuiltinActorsBundle(version actorstypes.Version, networkBundleName string) ([]byte, bool) {
	...
	fmt.Printf("GetEmbeddedBuiltinActorsBundle targetFileName: %s\n", targetFileName)
	for {
		header, err := tarReader.Next()
		switch err {
		case io.EOF:
		...
		fmt.Printf("GetEmbeddedBuiltinActorsBundle header.Name: %s\n", header.Name)
