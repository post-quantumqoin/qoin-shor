	...
	Action: func(cctx *cli.Context) error {
		...
		if len(pending) > 0 {
			...
			for _, txid := range txids {
				...
				if err != nil {
					if tx.Method == 0 {
						fmt.Fprintf(w, "%d\t%s\t%d\t%s\t%s\t%s(%d)\t%s\n", txid, "pending", len(tx.Approved), target, types.QOIN(tx.Value), "Send", tx.Method, paramStr)
					} else {
						fmt.Fprintf(w, "%d\t%s\t%d\t%s\t%s\t%s(%d)\t%s\n", txid, "pending", len(tx.Approved), target, types.QOIN(tx.Value), "new account, unknown method", tx.Method, paramStr)
					}
				} else {
					...
					fmt.Fprintf(w, "%d\t%s\t%d\t%s\t%s\t%s(%d)\t%s\n", txid, "pending", len(tx.Approved), target, types.QOIN(tx.Value), method.Name, tx.Method, paramStr)
				}
			}
...
var msigVestedCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		fmt.Printf("Vested: %s between %d and %d\n", types.QOIN(ret), start.Height(), end.Height())
