...
func infoCmdAct(cctx *cli.Context) error {
	...
	if err == nil {
		...
		fmt.Printf("      %s [%s]\n", addr.String(), types.QOIN(balance).Short())
	} else {
	...
	fmt.Printf("      Market locked: %s\n", types.QOIN(mbLockedSum).Short())
	fmt.Printf("      Market available: %s\n", types.QOIN(mbAvailableSum).Short())
...
func SyncBasefeeCheck(ctx context.Context, fullapi v1api.FullNode) error {
	...
	fmt.Printf(" [basefee %s]", color.New(gasCol...).Sprint(types.QOIN(basefee).Short()))
	return nil
}
