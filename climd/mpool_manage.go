...
func (mi *msgInfo) Row() []string {
	...
	return []string{"…" + cidStr[len(cidStr)-32:], shortAddr,
		fmt.Sprintf("%d", mi.sm.Message.Nonce), types.QOIN(mi.sm.Message.Value).String(),
...
func (mm *mmUI) messageDetail(mi msgInfo) func(*imtui.Tui) error {
	...
	return func(t *imtui.Tui) error {
		...
		display("Value: %s", types.QOIN(m.Value))
		row++
		display("GasLimit: %d", m.GasLimit)
		display("GasPremium: %s", types.QOIN(m.GasPremium).Short())
		display("GasFeeCap %s", types.QOIN(m.GasFeeCap).Short())
		row++
