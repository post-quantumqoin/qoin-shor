...
func feeUI(baseFee abi.TokenAmount, gasLimit int64, maxFee *abi.TokenAmount, send *bool) func(*imtui.Tui) error {
	...
	price := fmt.Sprintf("%s", types.QOIN(*maxFee).Unitless())
	...
	return func(t *imtui.Tui) error {
		if t.CurrentKey != nil {
			if t.CurrentKey.Key() == tcell.KeyRune {
				...
				case 's', 'S':
					price = types.QOIN(safe).Unitless()
				case '+':
					if err == nil {
						p := big.Mul(big.Int(pF), types.NewInt(11))
						p = big.Div(p, types.NewInt(10))
						price = fmt.Sprintf("%s", types.QOIN(p).Unitless())
					}
				case '-':
					if err == nil {
		...
		t.Label(0, row, fmt.Sprintf("Your configured maximum fee is: %s QOIN",
			types.QOIN(orignalMaxFee).Unitless()), defS)
		row++
		t.Label(0, row, fmt.Sprintf("Required maximum fee for the message: %s QOIN",
			types.QOIN(required).Unitless()), defS)
		row++
		w := t.Label(0, row, fmt.Sprintf("Safe maximum fee for the message: %s QOIN",
			types.QOIN(safe).Unitless()), defS)
		...
		*maxFee = abi.TokenAmount(pF)
		...
		t.Label(0, row, fmt.Sprintf("Current Base Fee is: %s", types.QOIN(baseFee).Nano()), defS)
		row++
		t.Label(0, row, fmt.Sprintf("Resulting FeeCap is: %s",
			types.QOIN(big.Div(*maxFee, big.NewInt(gasLimit))).Nano()), defS)
		row++
