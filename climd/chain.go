...
var ChainCmd = &cli.Command{
	Name:  "chain",
	Usage: "Interact with qoin blockchain",
	...
	Action: func(cctx *cli.Context) error {
		...
		if cctx.Bool("gas-stats") {
			...
			for i, ts := range tss {
				...
				afmt.Printf("%d: %d blocks (baseFee: %s -> maxFee: %s)\n", ts.Height(), len(ts.Blocks()), ts.Blocks()[0].ParentBaseFee, types.QOIN(types.BigMul(pbf, types.NewInt(uint64(build.BlockGasLimit)))))
...
var ChainGetCmd = &cli.Command{
   ...
   shor chain get /ipfs/[cid]/some/path
...
var ChainBisectCmd = &cli.Command{
   ...
   shor chain bisect [min height] [max height] '1/2/3/state/path' 'shell command' 'args'
   ...
   Example: find height at which deal ID 100 000 appeared
    - shor chain bisect 1 32000 '@Ha:t03/1' jq -e '.[2] > 100000'
