...
import (
	...
	lpaych "github.com/post-quantumqoin/qoin-shor/core/contracts/builtin/paych"
...
func paychStatus(writer io.Writer, avail *lapi.ChannelAvailableFunds) {
	...
	nameValues := [][]string{
		...
		{"To", avail.To.String()},
		{"Confirmed Amt", fmt.Sprintf("%s", types.QOIN(avail.ConfirmedAmt))},
		{"Available Amt", fmt.Sprintf("%s", types.QOIN(avail.NonReservedAmt))},
		{"Voucher Redeemed Amt", fmt.Sprintf("%s", types.QOIN(avail.VoucherReedeemedAmt))},
		{"Pending Amt", fmt.Sprintf("%s", types.QOIN(avail.PendingAmt))},
		{"Pending Available Amt", fmt.Sprintf("%s", types.QOIN(avail.PendingAvailableAmt))},
		{"Queued Amt", fmt.Sprintf("%s", types.QOIN(avail.QueuedAmt))},
	}
	if avail.PendingWaitSentinel != nil {
...
func outputVoucher(w io.Writer, v *paych.SignedVoucher, export bool) error {
	...
	fmt.Fprintf(w, "Lane %d, Nonce %d: %s", v.Lane, v.Nonce, types.QOIN(v.Amount))
	if export {
		fmt.Fprintf(w, "; %s", enc)
	}
	fmt.Fprintln(w)
