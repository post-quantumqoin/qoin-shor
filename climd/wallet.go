...
var walletVerify = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		data, err := hex.DecodeString(strings.TrimSpace(string(cctx.Args().Get(2))))
		if err != nil {
			return err
		}
...
var walletDelete = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		fmt.Println("Hard deletion of the address in `~/.shor/keystore` is needed for permanent removal")
...
var walletMarketWithdraw = &cli.Command{
	...
	ArgsUsage: "[amount (Qoin) optional, otherwise will withdraw max available]",
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:    "wallet",
	...
	Action: func(cctx *cli.Context) error {
		...
		notEnoughErr := func(msg string) error {
			return xerrors.Errorf("%s; "+
				...
				msg, types.QOIN(avail), types.QOIN(bal.Escrow), types.QOIN(bal.Locked), types.QOIN(reserved))
		}
		...
		if amt.GreaterThan(avail) {
			msg := fmt.Sprintf("can't withdraw more funds than available; requested: %s", types.QOIN(amt))
			return notEnoughErr(msg)
		}

		fmt.Printf("Submitting WithdrawBalance message for amount %s for address %s\n", types.QOIN(amt), wallet.String())
