...
func (pmgr *PeerMgr) doExpand(ctx context.Context) {
	...
	log.Info("dht doExpand")

	// routingDiscovery := routingdisc.NewRoutingDiscovery(pmgr.dht)
	// routingDiscovery.Advertise(ctx, "qoin/dht")
	// log.Info("Advertise qoin/dht")
	// peerChan, err := routingDiscovery.FindPeers(ctx, "qoin/dht", discovery.Limit(MaxFilPeers))
	// if err != nil {
	// 	log.Errorf("routingDiscovery find peers failed")
	// 	return
	// }
	...
	// 	select {
	// 	case per := <-peerChan:
	// 		if per.ID.Validate() == peer.ErrEmptyPeerID || per.ID == pmgr.h.ID() {
	// 			// log.Info("find peers is nil")
	// 			continue
	// 		}
	// 		log.Info("connecting to peers")
	// 		wg := sync.WaitGroup{}
	// 		wg.Add(1)
	// 		go func(per peer.AddrInfo) {
	// 			defer wg.Done()
	// 			if err := pmgr.h.Connect(ctx, per); err != nil {
	// 				log.Warnf("failed to connect to find peer: %s", err)
	// 			}
	// 		}(per)
	// 		wg.Wait()
	// 	case <-ctx.Done():
