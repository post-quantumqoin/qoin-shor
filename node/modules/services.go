...
import (
	...
	"github.com/post-quantumqoin/qoin-shor/core/sub"
...
func waitForSync(stmgr *stmgr.StateManager, epochs int, subscribe func()) {
	...
	log.Infow("waitForSync:", "timestampTime:", timestampTime, "nearsync", nearsync)
	//TODO:
...
func HandleDevNetIncomingBlocks(mctx helpers.MetricsCtx,
	lc fx.Lifecycle,
	ps *pubsub.PubSub,
	s *chain.Syncer,
	...
	if err := ps.RegisterTopicValidator(build.BlocksTopic("testnetnet"), v.Validate); err != nil {
		panic(err)
	}

	log.Infof("subscribing to pubsub topic %s", build.BlocksTopic("testnetnet"))

	blocksub, err := ps.Subscribe(build.BlocksTopic("testnetnet")) //nolint
	if err != nil {
		panic(err)
	}
