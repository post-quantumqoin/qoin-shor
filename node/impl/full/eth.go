...
func (a *EthModule) EthGetTransactionReceiptLimited(ctx context.Context, c cid.Cid, limit abi.ChainEpoch) (*api.EthTxReceipt, error) {
	...
	tx, err := newEthTxFromMessageLookup(ctx, msgLookup, -1, a.Chain, a.StateAPI)
	if err != nil {
		return nil, xerrors.Errorf("failed to convert %s into an Eth Txn: %w", c, err)
	}
	...
	if rct := msgLookup.Receipt; rct.EventsRoot != nil {
		...
		if err != nil {
			return nil, xerrors.Errorf("failed get events for %s", c)
		}
	}

	receipt, err := newEthTxReceipt(ctx, tx, msgLookup, events, a.Chain, a.StateAPI)
	if err != nil {
		return nil, xerrors.Errorf("failed to convert %s into an Eth Receipt: %w", c, err)
	}
...
func (a *EthModule) EthGetBalance(ctx context.Context, addr address.Address, blkParam ethtypes.EthBlockNumberOrHash) (ethtypes.EthBigInt, error) {
	...
	// }
	// var s string
	// if err := json.Unmarshal(addressRaw, &s); err != nil {
	// 	// return err
	// }
	// var addr address.Address

	// // addr.UnmarshalJSON(addressRaw)
	// if err := addr.UnmarshalJSON(addressRaw); err != nil {
	// 	// return xerrors.Errorf("failed to unmarshal storage slot: %w", err)
	// }
...
func (a *EthModule) EthMaxPriorityFeePerGas(ctx context.Context) (ethtypes.EthBigInt, error) {
	//TODO:BlockGasLimit Need for adjustment
	gasPremium, err := a.GasAPI.GasEstimateGasPremium(ctx, 0, builtinactors.SystemActorAddr, build.BlockGasLimit, types.EmptyTSK)
	if err != nil {
...
func (a *EthModule) EthGetMessageCid(ctx context.Context, rawTx jsonrpc.RawParams) ([]byte, error) {
	msg, err := jsonrpc.DecodeParams[ethtypes.QoinMessage](rawTx)
	if err != nil {
		return nil, xerrors.Errorf("decoding params: %w", err)
	}
	log.Warn("EthGetMessageCid msg From:", msg.From)
	...
	log.Warn("EthGetMessageCid unsignedMsg.Cid().Bytes():", unsignedMsg.Cid().Bytes())
