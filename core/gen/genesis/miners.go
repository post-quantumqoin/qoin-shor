...
func SetupStorageMiners(ctx context.Context, cs *store.ChainStore, sys vm.SyscallBuilder, sroot cid.Cid, miners []genesis.Miner, nv network.Version, synthetic bool) (cid.Cid, error) {
	...
	for i, m := range miners {
		...
		log.Info("miner.SealProofTypeFromSectorSize m.SectorSize: spt:", m.SectorSize, spt)
		if err != nil {
...

		{
			...
			log.Info("power CreateMiner from address: params Owner:  params Worker: ", m.Owner, m.Owner, m.Worker)
			...
			log.Info("minerInfos IDAddress: RobustAddress:", ma.IDAddress, ma.RobustAddress)
			...
			log.Info("minerInfos GetProvingPeriodStart:", pps)
			minerInfos[i].presealExp = (maxPeriods-1)*miner0.WPoStProvingPeriod + pps - 1
		}

		// Add market funds
		log.Info("Add market funds")
		...
		log.Info("Publish preseal deals, and calculate the QAPower")
		{
			publish := func(params *markettypes.PublishStorageDealsParams) error {
				fmt.Printf("publishing %d storage deals on client %s storage deals on miner %s with worker %s\n", len(params.Deals), params.Deals[0].Proposal.Client, params.Deals[0].Proposal.Provider, m.Worker)
			...
			for _, presealTmp := range m.Sectors {
				...
				p := markettypes.ClientDealProposal{
					...
					ClientSignature: crypto.Signature{Type: crypto.SigTypeMultiPqc},
				}
				...
				if av >= actorstypes.Version8 {
					...
					sig, err := sigs.MultiPqcSign(pqckey.PQCCert, pqckey.PqcKeypairs, buf)
				...
				fmt.Println("len(params.Deals)", len(params.Deals)) //len(params.Deals) 1
				...
				log.Info("m.ID m.SectorSize: sectorWeight: preseal.Deal.PieceSize:", m.ID, m.SectorSize, sectorWeight, preseal.Deal.PieceSize)
			}
	...
	{
		log.Info("power.Address SetTotalQualityAdjPower SetTotalRawBytePower qaPow: rawPow:", qaPow, rawPow)
		nh, err := genesisVm.Flush(ctx)
		if err != nil {
			return cid.Undef, xerrors.Errorf("flushing vm: %w", err)
		}
	...
	for i, m := range miners {
		// Commit sectors
		{
			for pi, preseal := range m.Sectors {
				...
				log.Info("Commit sectors preseal:")
				...
				log.Info("Commit sectors SetTotalRawBytePower pi: pc.RawBytePower: m.SectorSize:", pi, pc.RawBytePower, types.NewInt(uint64(m.SectorSize)))
				if err = pst.SetTotalRawBytePower(types.BigSub(pc.RawBytePower, types.NewInt(uint64(m.SectorSize)))); err != nil {
					return cid.Undef, xerrors.Errorf("setting TotalRawBytePower in power state: %w", err)
				}
				log.Info("Commit sectors SetTotalQualityAdjPower pi: pc.QualityAdjPower: sectorWeight:", pi, pc.QualityAdjPower, sectorWeight)
				...
				log.Info("Commit sectors currentEpochBlockReward minerInfos[i].maddr:", minerInfos[i].maddr)
				...
				log.Info("Commit sectors preCommitMethodNum minerInfos[i].maddr: from: value:", minerInfos[i].maddr, m.Worker, pledge)
				...
				log.Info("Commit sectors power.Address ConfirmSectorProofsValid minerInfos[i].maddr: from:", minerInfos[i].maddr, power.Address)
				...
				if av >= actorstypes.Version2 {
					...
					log.Info("Commit sectors power.Address UpdateClaimedPower minerInfos[i].maddr: claimParams.RawByteDelta: QualityAdjustedDelta:", minerInfos[i].maddr, claimParams.RawByteDelta, claimParams.QualityAdjustedDelta)
	...
	log.Info("Sanity-check total network power pc.RawBytePower:", pc.RawBytePower)
	...
	log.Info("Sanity-check total network power pc.QualityAdjPower:", pc.QualityAdjPower)
