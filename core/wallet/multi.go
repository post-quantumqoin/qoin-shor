...
type MultiWallet struct {
	...
	Pqc    *pqcwallet.PqcWallet       `optional:"true"`
...
func (m MultiWallet) WalletNew(ctx context.Context, keyType types.KeyType) (address.Address, error) {
	...
	if keyType == types.KTBLS || keyType == types.KTSecp256k1 {
		local = m.Local
	}
	log.Info("MultiWallet WalletNew")
	// return m.Pqc.WalletNew(ctx, keyType)
...
func (m MultiWallet) WalletHas(ctx context.Context, address address.Address) (bool, error) {
	w, err := m.find(ctx, address, m.Remote, m.Ledger, m.Local, m.Pqc)
	return w != nil, err
}
...
func (m MultiWallet) WalletList(ctx context.Context) ([]address.Address, error) {
	...
	ws := nonNil(m.Pqc, m.Remote, m.Ledger, m.Local)
	for _, w := range ws {
...
func (m MultiWallet) WalletSign(ctx context.Context, signer address.Address, toSign []byte, meta api.MsgMeta) (*crypto.Signature, error) {
	w, err := m.find(ctx, signer, m.Remote, m.Pqc, m.Ledger, m.Local)
	if err != nil {
		return nil, err
	}
	if w == nil {
...
func (m MultiWallet) WalletExport(ctx context.Context, addr address.Address) (*types.KeyInfo, error) {
	w, err := m.find(ctx, addr, m.Remote, m.Local, m.Pqc)
	if err != nil {
		return nil, err
	}
	if w == nil {
...
func (m MultiWallet) WalletImport(ctx context.Context, info *types.KeyInfo) (address.Address, error) {
	...
	if info.Type == types.KTBLS || info.Type == types.KTSecp256k1 {
		local = m.Local
	}
	w := firstNonNil(m.Remote, local)
	if w == nil {
...
func (m MultiWallet) WalletDelete(ctx context.Context, address address.Address) error {
	for {
		w, err := m.find(ctx, address, m.Remote, m.Pqc, m.Ledger, m.Local)
		if err != nil {
			return err
		}
		if w == nil {
