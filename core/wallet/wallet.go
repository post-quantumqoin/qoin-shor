...
func (w *LocalWallet) WalletHas(ctx context.Context, addr address.Address) (bool, error) {
	log.Debug("LocalWallet WalletHas address:", addr)
	k, err := w.findKey(addr)
	if err != nil {
		return false, err
	}
	log.Debug("LocalWallet WalletHas address is existence:", k != nil)
	return k != nil, nil
}
