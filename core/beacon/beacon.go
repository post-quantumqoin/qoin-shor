...
func ValidateBlockValues(bSchedule Schedule, nv network.Version, h *types.BlockHeader, parentEpoch abi.ChainEpoch, prevEntry types.BeaconEntry) error {
	...
	fmt.Println("ValidateBlockValues", "maxRound", maxRound)
	...
	fmt.Println("ValidateBlockValues", "prevEntry", prevEntry)
	fmt.Println("ValidateBlockValues", "h.BeaconEntries", h.BeaconEntries)
...
func BeaconEntriesForBlock(ctx context.Context, bSchedule Schedule, nv network.Version, epoch abi.ChainEpoch, parentEpoch abi.ChainEpoch, prev types.BeaconEntry) ([]types.BeaconEntry, error) {
	...
	// fmt.Println("BeaconEntriesForBlock", "parentBeacon", parentBeacon, "currBeacon", currBeacon)
	if parentBeacon != currBeacon && currBeacon.IsChained() {
		// Fork logic
		fmt.Println("BeaconEntriesForBlock", "currBeacon.IsChained() epoch:", epoch)
	...
	// fmt.Println("BeaconEntriesForBlock", "maxRound", maxRound, "start", start)
	...
	// fmt.Println("BeaconEntriesForBlock", "currEpoch", epoch, "parentEpoch", parentEpoch)
	for currEpoch := epoch; currEpoch > parentEpoch; currEpoch-- {
		...
		// fmt.Println("BeaconEntriesForBlock", "currEpoch", currEpoch)
		select {
		case resp := <-rch:
			if resp.Err != nil {
