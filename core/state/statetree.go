...
import (
	...
	builtin_types "github.com/post-quantumqoin/core-types/builtin"
	"github.com/post-quantumqoin/core-types/network"
	states0 "github.com/post-quantumqoin/specs-contracts/contracts/states"
	states2 "github.com/post-quantumqoin/specs-contracts/contracts/states"
	states3 "github.com/post-quantumqoin/specs-contracts/contracts/states"
	states4 "github.com/post-quantumqoin/specs-contracts/contracts/states"
	states5 "github.com/post-quantumqoin/specs-contracts/contracts/states"
...
func LoadStateTree(cst cbor.IpldStore, c cid.Cid) (*StateTree, error) {
	// ctx, span := trace.StartSpan(ctx, "LoadStateTree") //nolint:staticcheck
	var root types.StateRoot
	log.Debugw("LoadStateTree")
...
func (st *StateTree) SetActor(addr address.Address, act *types.Actor) error {
	// ctx, span := trace.StartSpan(ctx, "stateTree.SetActor") //nolint:staticcheck
	log.Debug("SetActor addr: ", addr)
...
func (st *StateTree) lookupIDinternal(addr address.Address) (address.Address, error) {
	log.Debug("lookupIDinternal addr: ", addr)
	act, err := st.GetActor(init_.Address)
	if err != nil {
		return address.Undef, xerrors.Errorf("getting init actor: %w", err)
	}
	log.Debug("lookupIDinternal init_.Load", act)
	...
	log.Debug("lookupIDinternal ias.ResolveAddress", a)
	if err == nil && !found {
...
func (st *StateTree) LookupID(addr address.Address) (address.Address, error) {
	if addr.Protocol() == address.ID {
		log.Debug("addr.Protocol() == address.ID", addr)
		return addr, nil
	}
	log.Debug("LookupID addr: ", addr)
	resa, ok := st.snaps.resolveAddress(addr)
	log.Debug("LookupID resa: ", resa)
	if ok {
		return resa, nil
	}
...
func (st *StateTree) GetActor(addr address.Address) (*types.Actor, error) {
	...
	addr = iaddr
	log.Debug("GetActor iaddr: ", iaddr)
	...
	if snapAct != nil {
		log.Debug("GetActor iaddr snapAct: ")
		return snapAct, nil
	}
	...
	if st.version <= types.StateTreeVersion4 {
		...
		log.Debug("st.version < st.root.Get: ", addr)
		found, err = st.root.Get(abi.AddrKey(addr), &act4)
		if found {
			act = *types.AsActorV5(&act4)
		}
		log.Debug("st.version < st.root.Get: ", found)
	} else {
		log.Debug("st.root.Get addr: ", addr)
		found, err = st.root.Get(abi.AddrKey(addr), &act)
		log.Debug("st.root.Get: ", found)
	}
	if err != nil {
...
func (st *StateTree) DeleteActor(addr address.Address) error {
	...
	log.Debug("DeleteActor addr: ", addr)
	iaddr, err := st.LookupID(addr)
	if err != nil {
		if xerrors.Is(err, types.ErrActorNotFound) {
