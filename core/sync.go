...
func (syncer *Syncer) syncFork(ctx context.Context, incoming *types.TipSet, known *types.TipSet, ignoreCheckpoint bool) ([]*types.TipSet, error) {
	...
	// forkLen := incoming.Height() - nts.Height()
	// log.Info("Sync fork len: ", int(forkLen), incoming.Height(), nts.Height())
	// if int(forkLen) > int(build.ForkLengthThreshold) {
	...
	at := incoming.Parents()
	for cunt := 0; cunt < int(build.ForkLengthThreshold); {
		ts, err := syncer.Exchange.GetBlocks(ctx, at, 500)
		if err != nil {
			return nil, err
		}
		cunt = cunt + 500
		log.Info("Sync fork got blocks: ", ts[0].Height(), len(ts), cunt)
		at = ts[len(ts)-1].Parents()
		tips = append(tips, ts...)
	}
...
func (syncer *Syncer) syncMessagesAndCheckState(ctx context.Context, headers []*types.TipSet) error {
	...
	return syncer.iterFullTipsets(ctx, headers, func(ctx context.Context, fts *store.FullTipSet) error {
		log.Debugw("validating tipset", "height", fts.TipSet().Height(), "cids", fts.TipSet().Cids())
		if err := syncer.ValidateTipSet(ctx, fts, false); err != nil {
...
func (syncer *Syncer) iterFullTipsets(ctx context.Context, headers []*types.TipSet, cb func(context.Context, *store.FullTipSet) error) error {
	...
	log.Debugw("iterFullTipsets", "len(headers)", int64(len(headers)))
	...
	for i := len(headers) - 1; i >= 0; {
		...
		log.Debugw("iterFullTipsets fetchMessages", "startOffset:", startOffset, "tartOffset+batchSize:", startOffset+batchSize)
...
func persistMessages(ctx context.Context, bs bstore.Blockstore, bst *exchange.CompactedMessages) error {
	...
	for _, m := range bst.Secpk {
		if m.Signature.Type != crypto.SigTypeSecp256k1 && m.Signature.Type != crypto.SigTypeDelegated && m.Signature.Type != crypto.SigTypeMultiPqc {
