...
func (filec *FilecoinEC) ValidateBlock(ctx context.Context, b *types.FullBlock) (err error) {
	...
	nbitCheck := async.Err(func() error {
		...
		if err := filec.nbitIsValid(ctx, h, baseTs); err != nil {
...
func (filec *FilecoinEC) IsEpochInConsensusRange(epoch abi.ChainEpoch) bool {
	...
	alloweph := abi.ChainEpoch((now-filec.genesis.MinTimestamp())/build.BlockDelaySecs) + abi.ChainEpoch(((now-filec.genesis.MinTimestamp())%build.BlockDelaySecs)/build.PropagationDelaySecs)
	log.Infow("allowable height range", "epoch:", epoch, "now time:", now,
		"now height:", (now-filec.genesis.MinTimestamp())/build.BlockDelaySecs,
		"range:", (now-filec.genesis.MinTimestamp())%build.BlockDelaySecs,
		"alloweph:", alloweph)
	if epoch > alloweph {
...
func (filec *FilecoinEC) nbitIsValid(ctx context.Context, h *types.BlockHeader, baseTs *types.TipSet) error {
	var newNbit []byte
	var cuTsNbit []byte
	lastTipSet := baseTs
	...
	if lastTipSet.Height() != 0 && lastTipSet.Height()%pqcpow.NbitSampleRate == 0 {
		// sampleTipSet, err := filec.store.LoadTipSet(ctx, lastTipSet.Parents())
		// if err != nil {
		// 	return err
		// }
