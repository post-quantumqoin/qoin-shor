...
func checkBlockMessages(ctx context.Context, sm *stmgr.StateManager, cs *store.ChainStore, b *types.FullBlock, baseTs *types.TipSet) error {
	...
	if err != nil {
		return xerrors.Errorf("failed to compute tipsetstate for %s: %w", baseTs.Key(), err)
	}
	log.Debugw("checkBlockMessages")
...
func CreateBlockHeader(ctx context.Context, sm *stmgr.StateManager, pts *types.TipSet, bt *api.BlockTemplate) (*types.BlockHeader, []*types.Message, []*types.SignedMessage, error) {
	...
	next := &types.BlockHeader{
		...
		PqcPowProof:   bt.Pproof,

		BeaconEntries:         bt.BeaconValues,
		Height:                bt.Epoch,
		Timestamp:             bt.Timestamp,
		MiningTimestamp:       bt.MiningTimestamp,
