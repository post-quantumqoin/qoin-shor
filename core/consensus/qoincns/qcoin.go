...
import (
	...
	builtin_ "github.com/post-quantumqoin/core-types/builtin"
...
var log = logging.Logger("qoin-consensus")

type QoinCons struct {
...
var RewardFunc = func(ctx context.Context, vmi vm.Interface, em stmgr.ExecMonitor,
	...
	rwMsg := &types.Message{
		...
		GasLimit:   1 << 30,
		Method:     reward.Methods.PqcBlockReward,
		Params:     ser,
	}
...
var PqcRewardFunc = func(ctx context.Context, vmi vm.Interface, em stmgr.ExecMonitor,
	epoch abi.ChainEpoch, ts *types.TipSet, params *reward.AwardBlockRewardParams) error {
	//
	ser, err := actors.SerializeParams(params)
	if err != nil {
	...
	rwMsg := &types.Message{
		...
		GasPremium: types.NewInt(0),
		GasLimit:   1 << 30,
		Method:     builtin_.MethodsReward.PqcBlockReward,
		Params:     ser,
	}
	ret, actErr := vmi.ApplyImplicitMessage(ctx, rwMsg)
	if actErr != nil {
...
func NewQoinExpectedConsensus(sm *stmgr.StateManager, beacon beacon.Schedule, verifier storiface.Verifier, genesis chain.Genesis) consensus.Consensus {
...
func (qcs *QoinCons) ValidateBlock(ctx context.Context, b *types.FullBlock) (err error) {
	...
	baseTs, err := qcs.store.LoadTipSet(ctx, types.NewTipSetKey(h.Parents...))
	if err != nil {
		return xerrors.Errorf("load parent tipset failed (%s): %w", h.Parents, err)
	}

	// winPoStNv := qcs.sm.GetNetworkVersion(ctx, baseTs.Height())

	// lbts, lbst, err := stmgr.GetLookbackTipSetForRound(ctx, qcs.sm, baseTs, h.Height)
	// if err != nil {
	// 	return xerrors.Errorf("failed to get lookback tipset for block: %w", err)
	// }

	// TODO: Optimization: See https://github.com/post-quantumqoin/qoin-shor/issues/11597
	prevBeacon, err := qcs.store.GetLatestBeaconEntry(ctx, baseTs)
	if err != nil {
		return xerrors.Errorf("failed to get latest beacon entry: %w", err)
	}
	fmt.Println("ValidateBlock", "prevEntry", prevBeacon)
	// fast checks first
	...
	nbitCheck := async.Err(func() error {
		fmt.Println("nbitCheck nbitIsValid")
		if err := qcs.nbitIsValid(ctx, h, baseTs); err != nil {
			return xerrors.Errorf("nbitIsValid failed: %w", err)
		}
		return nil
	})

	pqcPowCheck := async.Err(func() error {
		// tc := h.Ticket
		// buf := new(bytes.Buffer)

		// if err := tc.MarshalCBOR(buf); err != nil {
		// 	return xerrors.Errorf("failed to marshal ticket to cbor: %w", err)
		// }
		// log.Infow("pqcPowCheck VerifyPoW", "h.PqcPowProof.PowProof: h.MiningTimestamp:", h.PqcPowProof.PowProof, h.MiningTimestamp)
		var buf []byte
		// if build.UpgradeYellowStoneHeight >= 0 && baseTs.Height() >= build.UpgradeYellowStoneHeight {
		buf, err = h.PqcProofSeed() //TODO:upgrade v2.0
		if err != nil {
			return xerrors.Errorf("ValidateBlock failed get qpcseed:: %w", err)
		}
		// } else {
		// 	buf, err = h.PqcSeed()
		// 	if err != nil {
		...
		// log.Infow("pqcPowCheck VerifyPoW", "h.PqcPowProof.PowProof: h.MiningTimestamp:", h.PqcPowProof.PowProof, h.MiningTimestamp)
		seed := shake3.Shake256XOF(buf, 72)
	...
	// 	if err := qcs.minerIsValid(ctx, h.Miner, baseTs); err != nil {
	// 		return xerrors.Errorf("minerIsValid failed: %w", err)
	// 	}
	// 	return nil
	// })

	pweight, err := qcs.store.Weight(ctx, baseTs)
	if err != nil {
	...
	// waddr, err := stmgr.GetMinerWorkerRaw(ctx, qcs.sm, lbst, h.Miner)
	// if err != nil {
	...
	// 	eligible, err := stmgr.MinerEligibleToMine(ctx, qcs.sm, h.Miner, baseTs, lbts)
	// 	if err != nil {
	...
	// 	slashed, err := stmgr.GetMinerSlashed(ctx, qcs.sm, baseTs, h.Miner)
	// 	if err != nil {
	...
	// 	mpow, tpow, _, err := stmgr.GetPowerRaw(ctx, qcs.sm, lbst, h.Miner)
	// 	if err != nil {
	...
	// 	fmt.Println("blockSigCheck verifyBlockSignature")
	...
	beaconValuesCheck := async.Err(func() error {
		...
		nv := qcs.sm.GetNetworkVersion(ctx, h.Height)
		if err := beacon.ValidateBlockValues(qcs.beacon, nv, h, baseTs.Height(), *prevBeacon); err != nil {
	...
	tktsCheck := async.Err(func() error {
		...
		ok := bytes.Equal(vrfBase, h.Ticket.VRFProof)
		if !ok {
	...
	// 	if err := qcs.VerifyWinningPoStProof(ctx, winPoStNv, h, *prevBeacon, lbst, waddr); err != nil {
	// 		return xerrors.Errorf("invalid election post: %w", err)
	// 	}
	// 	return nil
	// })

	commonChecks := consensus.CommonBlkChecks(ctx, qcs.sm, qcs.store, b, baseTs)
...
func blockSanityChecks(h *types.BlockHeader) error {
	...
	// if h.PqcPowProof == nil {
	// 	return xerrors.Errorf("block cannot have nil Pqc work proof")
	// }
...
func (qcs *QoinCons) VerifyWinningPoStProof(ctx context.Context, nv network.Version, h *types.BlockHeader, prevBeacon types.BeaconEntry, lbst cid.Cid, waddr address.Address) error {
	if build.InsecurePoStValidation {
	...
	xsectors, err := stmgr.GetSectorsForWinningPoSt(ctx, nv, qcs.verifier, qcs.sm, lbst, h.Miner, rand)
	if err != nil {
...
func (qcs *QoinCons) IsEpochInConsensusRange(epoch abi.ChainEpoch) bool {
	if qcs.genesis == nil {
		return true
	}
	...
	if epoch < qcs.store.GetHeaviestTipSet().Height()-build.Finality {
		return false
	}

	now := uint64(build.Clock.Now().Unix())

	// No new blocks are allowed to be generated during network propagation time
	alloweph := abi.ChainEpoch((now-qcs.genesis.MinTimestamp())/build.BlockDelaySecs) + abi.ChainEpoch(((now-qcs.genesis.MinTimestamp())%build.BlockDelaySecs)/build.PropagationDelaySecs)
	log.Infow("allowable height range", "epoch:", epoch, "now time:", now,
		"now height:", (now-qcs.genesis.MinTimestamp())/build.BlockDelaySecs,
		"range:", (now-qcs.genesis.MinTimestamp())%build.BlockDelaySecs,
		"alloweph:", alloweph)
	if epoch > alloweph {
		log.Warnw("epoch exceeds the allowable height range:", "epoch:", epoch, "alloweph:", alloweph)
		return false
	}

	return epoch <= (abi.ChainEpoch((now-qcs.genesis.MinTimestamp())/build.BlockDelaySecs) + MaxHeightDrift)
}

func (qcs *QoinCons) minerIsValid(ctx context.Context, maddr address.Address, baseTs *types.TipSet) error {
	act, err := qcs.sm.LoadActor(ctx, power.Address, baseTs)
	if err != nil {
		return xerrors.Errorf("failed to load power actor: %w", err)
	}

	powState, err := power.Load(qcs.store.ActorStore(ctx), act)
	if err != nil {
...
func (qcs *QoinCons) nbitIsValid(ctx context.Context, h *types.BlockHeader, baseTs *types.TipSet) error {
	var newNbit []byte
	var cuTsNbit []byte
	lastTipSet := baseTs
	...
	if lastTipSet.Height() != 0 && lastTipSet.Height()%pqcpow.NbitSampleRate == 0 {
		...
		startTime := lastTipSet.MinTimestamp()
		endTime := lastTipSet.MinTimestamp() + lastTipSet.MinMiningTimestamp()
		log.Infow("nbitIsValid ", "lastNbit:", lastNbit, "lastTipSet.Height: ",
		...
		nulls := h.Height - (lastTipSet.Height() + 1) //TODO:upgrade v2.0
		if nulls > 0 {
			endTime = startTime + uint64(nulls)*15 //15s
		}
		...
		log.Infow("GetNbit ", "lastNbit:", lastNbit, "lastTipSet.Height: ", lastTipSet.Height())
		newNbit = lastNbit
		cuTsNbit = h.PqcPowProof.Nbit
	}
	// var cuTsNbit []byte
	// if len(h.PqcPowProof.Nbit) <= 3 {
	// 	cuTsNbit = h.PqcPowProof.Nbit
	// } else {
	// 	cuTsNbit = h.PqcPowProof.Nbit[:3] //TODO:upgrade v2.0
	// }

	fmt.Println("cuTsNbit: newNbit:", cuTsNbit, newNbit)

	ok := bytes.Equal(cuTsNbit, newNbit)
	if !ok {
		return xerrors.Errorf("validating block Nbit failed cuTsNbit: %w newNbit:%w ", cuTsNbit, newNbit)
	}
...
func VerifyVRF(ctx context.Context, worker address.Address, vrfBase, vrfproof []byte) error {
	...
	// 	// Data: vrfproof,
	// }
	fmt.Println("computeTicket vrfproof", "len", len(vrfproof))
	var sig crypto.Signature
	if err := sig.UnmarshalCBOR(bytes.NewReader(vrfproof)); err != nil {
		return xerrors.Errorf("unmarshaling error data: %w", err)
	}
	if err := sigs.MultiPqcVerify(&sig, worker, vrfBase); err != nil {
...
func (qcs *QoinCons) ValidateBlockHeader(ctx context.Context, b *types.BlockHeader) (rejectReason string, err error) {
	...
	// key, err := qcs.checkPowerAndGetWorkerKey(ctx, b)
	// if err != nil {
	// 	if err != ErrSoftFailure && qcs.isChainNearSynced() {
...
func (qcs *QoinCons) checkPowerAndGetWorkerKey(ctx context.Context, bh *types.BlockHeader) (address.Address, error) {
	// we check that the miner met the minimum power at the lookback tipset

	baseTs := qcs.store.GetHeaviestTipSet()
	lbts, lbst, err := stmgr.GetLookbackTipSetForRound(ctx, qcs.sm, baseTs, bh.Height)
	if err != nil {
		log.Warnf("failed to load lookback tipset for incoming block: %s", err)
		return address.Undef, ErrSoftFailure
	}

	key, err := stmgr.GetMinerWorkerRaw(ctx, qcs.sm, lbst, bh.Miner)
	if err != nil {
	...
	eligible, err := stmgr.MinerEligibleToMine(ctx, qcs.sm, bh.Miner, baseTs, lbts)
	if err != nil {
...
func (qcs *QoinCons) isChainNearSynced() bool {
	ts := qcs.store.GetHeaviestTipSet()
...
var _ consensus.Consensus = &QoinCons{}
