...
import (
	...
	tbig "github.com/post-quantumqoin/core-types/big"
	"github.com/post-quantumqoin/core-types/crypto"

	"github.com/post-quantumqoin/qoin-shor/build"
	"github.com/post-quantumqoin/qoin-shor/core/messagepool/gasguess"
...
func (sm *selectedMessages) tryToAdd(mc *msgChain) bool {
	...
	} else if mc.sigType == crypto.SigTypeSecp256k1 || mc.sigType == crypto.SigTypeDelegated || mc.sigType == crypto.SigTypeMultiPqc {
		if sm.secpLimit < l {
			return false
		}
...
func (sm *selectedMessages) trimChain(mc *msgChain, mp *MessagePool, baseFee types.BigInt) {
	...
	if mc.gasLimit > sm.gasLimit || len(mc.msgs) > msgLimit {
		log.Infow("selectedMessages trimChain", "mc.gasLimit", mc.gasLimit, "len(mc.msgs)", len(mc.msgs))
...
func (mp *MessagePool) selectMessagesOptimal(ctx context.Context, curTs, ts *types.TipSet, tq float64) (*selectedMessages, error) {
	...
	log.Infow("selectMessagesOptimal", "baseFee:", baseFee)
	...
	if len(chains) != 0 && chains[0].gasPerf < 0 {
		log.Warnw("all messages in mpool have non-positive gas performance", "bestGasPerf", chains[0].gasPerf, "chains[0].msgs[0].Message.From:", chains[0].msgs[0].Message.From)
		return result, nil
	}
	...
	log.Info("selectMessagesOptimal blockProbabilities")
	...
	log.Info("selectMessagesOptimal BeforeEffective")
	...
	log.Info("selectMessagesOptimal startMerge")
	startMerge := time.Now()
	last := len(chains)
	...
	startTail := time.Now()
	log.Infow("selectMessagesOptimal", "chains len", len(chains))
tailLoop:
	...
	log.Info("selectMessagesOptimal tailLoop result.gasLimit >= minGas && last < len(chains) ")
	...
	if result.gasLimit >= minGas && len(result.msgs) <= build.BlockMessageLimit {
		...
		log.Info("selectMessagesOptimal tailLoop result.gasLimit >= minGas && len(result.msgs) <= build.BlockMessageLimit ")
...
func (*MessagePool) getGasReward(msg *types.SignedMessage, baseFee types.BigInt) *big.Int {
	...
	log.Infow("getGasReward", "msg.Message.From:", msg.Message.From, "msg.Message.To:", msg.Message.To,
		"msg.Message.GasFeeCap:", msg.Message.GasFeeCap, "baseFee", baseFee, "gasReward", gasReward, "maxPremium", maxPremium,
		"msg.Message.GasPremium", msg.Message.GasPremium)
...
func (mp *MessagePool) createMessageChains(actor address.Address, mset map[uint64]*types.SignedMessage, baseFee types.BigInt, ts *types.TipSet) []*msgChain {
	...
	newChain := func(m *types.SignedMessage, i int) *msgChain {
		...
		log.Infow("newChain", "chain.msgs[0].Message.From", chain.msgs[0].Message.From, "gasReward", chain.gasReward, "chain.gasLimit", chain.gasLimit, "chain.gasPerf", chain.gasPerf)
		return chain
	}
	...
	for i, m := range msgs {
		...
		// log.Infow("createMessageChains", "gasReward:", gasReward, "curChain.gasReward:", curChain.gasReward, "rewards[i]:", rewards[i])
		gasLimit := curChain.gasLimit + m.Message.GasLimit
		// log.Infow("createMessageChains", "gasLimit:", gasLimit, "curChain.gasLimit:", curChain.gasLimit, "m.Message.GasLimit:", m.Message.GasLimit)
		gasPerf := mp.getGasPerf(gasReward, gasLimit)
		// log.Infow("createMessageChains", "m.Message.From:", m.Message.From, "curChain.gasPerf:", curChain.gasPerf, "gasPerf:", gasPerf, "gasLimit:", gasLimit, "gasReward:", gasReward)
