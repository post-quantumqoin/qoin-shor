...
func (sm *StateManager) callInternal(ctx context.Context, msg *types.Message, priorMsgs []types.ChainMsg, ts *types.TipSet, stateCid cid.Cid, nvGetter rand.NetworkVersionGetter, checkGas, applyTsMessages bool) (*api.InvocResult, error) {
	...
	log.Debugw("StateManager callInternal")
	...
	if checkGas {
		...
		case address.Delegated:
			msgApply = &types.SignedMessage{
				...
				Signature: crypto.Signature{
					...
					Data: make([]byte, crypto.MultiPqcSigLen),
				},
			}
		case address.PQC:
			msgApply = &types.SignedMessage{
		...
		default:
			return nil, xerrors.New("unrecognized address types")
		}

		ret, err = vmi.ApplyMessage(ctx, msgApply)
		if err != nil {
			return nil, xerrors.Errorf("gas estimation failed: %w addr:%w", err, fromKey.Protocol())
		}
