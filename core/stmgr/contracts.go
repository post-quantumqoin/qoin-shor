...
func MinerGetBaseInfo(ctx context.Context, sm *StateManager, bcs beacon.Schedule, tsk types.TipSetKey, round abi.ChainEpoch, maddr address.Address, pv storiface.Verifier) (*api.MiningBaseInfo, error) {
	...
	// fmt.Println("MinerGetBaseInfo BeaconEntriesForBlock round: ts.Height():", round, ts.Height())
	var entries []types.BeaconEntry
	// if build.UpgradeYellowStoneHeight >= 0 && ts.Height() == build.UpgradeYellowStoneHeight {
	// 	currBeacon := bcs.BeaconForEpoch(round)
	// 	currRound := currBeacon.MaxBeaconRoundForEpoch(sm.GetNetworkVersion(ctx, round), round)
	// 	rch := currBeacon.Entry(ctx, currRound)
	// 	fmt.Println("MinerGetBaseInfo", "round", round)
	// 	select {
	// 	case resp := <-rch:
	// 		if resp.Err != nil {
	...
	// 	case <-ctx.Done():
	// 		return nil, xerrors.Errorf("context timed out waiting on beacon entry to come back for epoch %d: %w", round, ctx.Err())
	// 	}
	// } else {
	...
	// fmt.Println("MinerGetBaseInfo prev:", prev, " entries:", entries)
	//
	fmt.Println("MinerGetBaseInfo maddr.Protocol(): address.ID:", maddr.Protocol(), address.ID)
	if maddr.Protocol() != address.ID {
		return &api.MiningBaseInfo{
			MinerPower:        big.NewInt(0),
			NetworkPower:      big.NewInt(0),
			Sectors:           nil,
			WorkerKey:         maddr,
			SectorSize:        0,
	...
	// fmt.Println("MinerGetBaseInfo maddr:", maddr)
	// return &api.MiningBaseInfo{
	// 	MinerPower:        big.NewInt(0),
	// 	NetworkPower:      big.NewInt(0),
	// 	Sectors:           nil,
	// 	WorkerKey:         maddr,
	// 	SectorSize:        abi.SectorSize(1073741824),
	// 	PrevBeaconEntry:   *prev,
	// 	BeaconEntries:     entries,
