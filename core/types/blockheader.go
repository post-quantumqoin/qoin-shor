...
// height nbit is defined:|equations[0]|threshold[1:2]|seed[2:34]|
type PqcPowProof struct {
	Nbit     []byte
	PowProof []byte
}
...
func (blk *BlockHeader) PqcSeed() ([]byte, error) {
	blkcopy := *blk
	blkcopy.BlockSig = nil
	blkcopy.MiningTimestamp = 0
	blkcopy.PqcPowProof = nil

	return blkcopy.Serialize()
}

func (blk *BlockHeader) PqcProofSeed() ([]byte, error) {
	blkcopy := *blk
	blkcopy.BlockSig = nil
	blkcopy.MiningTimestamp = 0
	// proofcopy := &PqcPowProof{
	// 	Nbit: blkcopy.PqcPowProof.Nbit,
	// }
	proofcopy := *blkcopy.PqcPowProof
	proofcopy.PowProof = nil

	blkcopy.PqcPowProof = &proofcopy
