...
func (ts *TipSet) MinMiningTimestamp() uint64 {
	if ts == nil {
		return 0
	}
	...
	minTs := blks[0].MiningTimestamp
	for _, bh := range blks[1:] {
		if bh.MiningTimestamp < minTs {
			minTs = bh.MiningTimestamp
		}
	}
	return minTs
}
...
func (ts *TipSet) PqcPowProof() *PqcPowProof {
	return ts.blks[0].PqcPowProof
}
