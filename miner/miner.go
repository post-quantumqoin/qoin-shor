...
minerLoop:
	for {
		...
		onDone(b != nil, h, nil)
		log.Info("Process the mined block")
		...
		if b != nil {
			...
			if os.Getenv("QOIN_MINER_NO_SLASHFILTER") != "_yes_i_know_i_can_and_probably_will_lose_all_my_fil_and_power_" && !build.IsNearUpgrade(base.TipSet.Height(), build.UpgradeWatermelonFixHeight) {
...
func (m *Miner) mineOne(ctx context.Context, base *MiningBase) (minedBlock *types.BlockMsg, err error) {
	...
	fmt.Println("mineOne ComputeProof")
	...
	fmt.Println("mineOne MpoolSelect")
	...
	fmt.Println("mineOne createBlock")
